    ...
    <div>
      ...
      <div className="container">
        <div id="content">
          ...
          <p>To see the code, please see my <a href="https://github.com/ifoulidis?tab=repositories" className='inTextLinks'>GitHub page</a> or view it in a <Link href="/UFC_project_code.pdf"><a className='inTextLinks'>PDF</a></Link>.</p>
          <p><br />The Ultimate Fighting Championship (UFC) is the premier mixed martial arts (MMA) organisation. Athletes typically use techniques kickboxing, wrestling, and Brazilian
            Jiu Jitsu (among other martial arts) to defeat their opponents. More information about mixed martial arts can be found in the <a className='inTextLinks'>UFC&apos;s Introduction to MMA.</a></p>
          <p><br />The dataset used here was kindly gathered by Rajeev Warrier. <a href="https://www.kaggle.com/datasets/rajeevw/ufcdata" className='inTextLinks'>Click here </a>to see a description of the dataset.</p>
          ...
          <p>Although the data are incomplete and have some issues, the results are quite interesting, in that the mean difference is very small. The strength of the correlation and statistical significance are irrelevant, since the differences are so small anyway. <br/>What I thought would be interesting to do next is find the average difference in reach for opponents of each weight class. The results were as follows:</p>
          ...
          <p>The data show that the average reach difference for every weight class is substantial, of which I was already aware.<br/>I also checked whether any reach advantage at all relates to win rate. The results were as follows:</p>
          ...
          <p>There are yet more findings to see, so check out the <Link href="/UFC_project_code.pdf"><a className='inTextLinks'>PDF</a></Link> if you are interested in this topic.</p>
        </div>
      </div>
    </div>
  )
